...
class FundProject(models.Model):
    ...
    # Balance fields - all computed, all readonly
    ...
    bill_ids = fields.One2many('nn.fund.bill', 'project_id', string='Bills')
    ...
    def _compute_balances(self):
        ...
        for rec in self:
            ...
            incoming_t = Transfer.search([('destination_project_id', '=', rec.id),
                                          ('state', '=', 'approved')])
            outgoing_t = Transfer.search([('source_project_id', '=', rec.id),
                                          ('state', '=', 'approved')])
    ...
    def _check_no_negative_balance(self):
        for rec in self:
            if rec.available_fund < -0.01:
                raise ValidationError(f'Project "{rec.name}" cannot have a negative available balance.')

    def unlink(self):
        for rec in self:
            if rec.allocation_ids or rec.requisition_ids:
                raise UserError('Cannot delete a project that has allocations or requisitions.')
        return super().unlink()
